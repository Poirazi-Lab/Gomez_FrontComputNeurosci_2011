	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  hoc_register_dparam_size(_mechtype, 5);
 	...
 	ivoc_help("help ?1 calH /home/jg/ModelosNeuron/ProgramsNeuronCA1_JG/CleanVersion_CA1_JG_15Mar09/mechanism/x86_64/calH.mod\n");
...
double varss (  _lv , _li )  
	double _lv , _li ;
 {
   double _lvarss;
 if ( _li  == 0.0 ) {
     _lvarss = 1.0 / ( 1.0 + exp ( ( _lv + 37.7 ) / ( - 1.0 ) ) ) ;
     }
   else if ( _li  == 1.0 ) {
...
static double _nrn_current(_v) double _v;{double _current=0.;v=_v;{ {
   ecan = ( 1e3 ) * ( R * ( celsius + 273.15 ) ) / ( 2.0 * FARADAY ) * log ( cao / cai ) ;
 ...
 v=_v;
{
 ...
 error =  states();
 if(error){fprintf(stderr,"at line 63 in file calH.mod:\n	SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
